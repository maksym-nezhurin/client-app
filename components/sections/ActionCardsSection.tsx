  ...
  return (
    <section className={`grid gap-6 ${user ? 'lg:grid-cols-2' : 'lg:grid-cols-3'}`}>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-[0_0_40px_rgba(15,23,42,0.5)] backdrop-blur">
        <h2 className="text-xl font-semibold">{t('actions.start_searching.title')}</h2>
        <p className="mt-2 text-sm text-slate-300">
          {t('actions.start_searching.description')}
        </p>
        <Button asChild className="mt-6 w-full">
          <Link href={ROUTES.BROWSE}>{t('actions.start_searching.cta')}</Link>
        </Button>
      </div>
      ...
      {showListCarActive ? (
        <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-[0_0_40px_rgba(15,23,42,0.5)] backdrop-blur">
          <h2 className="text-xl font-semibold">{t('actions.list_car.title')}</h2>
          <p className="mt-2 text-sm text-slate-300">{t('actions.list_car.description')}</p>
          <Button 
            asChild 
            variant="secondary" 
          ...
          >
            {user ? (
              <a href={ROUTES.SELL + '#list-form'} onClick={handleListCarClick}>
                {t('actions.list_car.cta')}
              </a>
            ) : (
              <Link href={ROUTES.SELL}>{t('actions.list_car.cta')}</Link>
            )}
          </Button>
        </div>
      ) : (
      ...
      {!user && (
        <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-[0_0_40px_rgba(15,23,42,0.5)] backdrop-blur">
          <h2 className="text-xl font-semibold">{t('actions.create_account.title')}</h2>
          <p className="mt-2 text-sm text-slate-300">
            {t('actions.create_account.description')}
          </p>
          <Button asChild variant="ghost" className="mt-6 w-full">
            <Link href={ROUTES.AUTH.SIGNUP}>{t('actions.create_account.cta')}</Link>
          </Button>
        </div>
      )}
    </section>
