      ...
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="mb-4 text-2xl font-bold">
            {t('cars.new_car.feature_disabled_title')}
          </h1>
          <p className="mb-6 text-muted-foreground">
            {reason === 'market_not_supported' ? (
              <>
                {t('cars.new_car.feature_disabled_subtitle')}{' '}
                <span className="font-semibold">{marketConfig.flag} {marketConfig.name}</span>.
                <br />
                {t('cars.new_car.feature_disabled_description')}
              </>
            ) : (
              t('cars.new_car.feature_disabled_title')
            )}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Button asChild variant="secondary">
              <Link href={ROUTES.ACCOUNT_CARS}>
                {t('cars.new_car.back_to_cars')}
              </Link>
            </Button>
            <Button asChild>
