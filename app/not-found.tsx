  ...
  return (
    <div className="flex min-h-[400px] flex-col items-center justify-center rounded-lg border border-muted bg-white p-8 text-center shadow">
      ...
      <h1 className="mb-2 text-2xl font-semibold text-text-primary">
        {t('client.account_menu.not_found_title') || 'Page Not Found'}
      </h1>
      <p className="mb-6 text-muted-foreground">
        {t('client.account_menu.not_found_description') || 
          'The account page you\'re looking for doesn\'t exist or has been moved.'}
      </p>
      <div className="flex gap-3">
        <Button asChild>
          <Link href={ROUTES.ACCOUNT}>
            {t('client.account_menu.back_to_dashboard') || 'Back to Dashboard'}
          </Link>
        </Button>
        <Button asChild variant="secondary">
          <Link href={ROUTES.HOME}>
            {t('client.account_menu.back_to_home') || 'Back to Home'}
          </Link>
        </Button>
      </div>
    </div>
